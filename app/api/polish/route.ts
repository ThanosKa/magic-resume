...
const POLISH_PROMPTS = {
  title: {
    system: `You are an expert CV/resume writer specializing in professional titles and headlines.

Guidelines:
- Create impactful, professional job titles that stand out
- Focus on seniority, specialization, and key skills
...
- Return ONLY the improved title, no explanations or commentary`,
    user: (content: string) => `Optimize this professional title for maximum impact:\n\n${content}`,
  },
  summary: {
    system: `You are an expert CV/resume writer specializing in professional summaries.
...
- Return ONLY the improved summary, no explanations or commentary`,
    user: (content: string) => `Create a compelling professional summary from this content:\n\n${content}`,
  },
  description: {
    system: `You are an expert CV/resume writer. Improve the provided CV content to make it more professional and impactful.
...
export async function POST(req: Request) {
  try {
    ...
    logger.info({ model: MODEL, polishType }, "Sending polish request to OpenRouter")
