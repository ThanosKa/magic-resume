            ...
            <View key={exp.id} style={styles.item}>
              <View style={styles.itemHeader}>
                ...
                {formatDateRange(exp.startDate, exp.endDate) && (
                  <Text style={styles.date}>{formatDateRange(exp.startDate, exp.endDate)}</Text>
                )}
              </View>
            ...
            <View key={project.id} style={styles.item}>
              <View style={styles.itemHeader}>
                <View style={{ flex: 1 }}>
                  ...
                  {project.link && (
                    <PdfLink src={project.link} style={[styles.itemSubtitle, styles.link]}>
                      {project.link}
                    </PdfLink>
                  )}
                </View>
...
export function CVPDFDocument({ cv }: CVPDFDocumentProps) {
  ...
  const orderedSections = sections.filter((s) => s.enabled).sort((a, b) => a.order - b.order)
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={styles.row}>
            {personalInfo.email && <Text style={styles.rowItem}>{personalInfo.email}</Text>}
            {personalInfo.phone && <Text style={styles.rowItem}>{personalInfo.phone}</Text>}
            {personalInfo.location && <Text style={styles.rowItem}>{personalInfo.location}</Text>}
          </View>

          {personalInfo.socialLinks && personalInfo.socialLinks.length > 0 && (
            <View style={styles.row}>
              {personalInfo.socialLinks.map((link) => (
                <PdfLink key={link.id} src={link.url} style={[styles.rowItem, styles.link]}>
